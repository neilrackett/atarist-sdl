...
const char ST_RENDER_GRAYSCALE[] = "grayscale";
const char ST_RENDER_COLOR[] = "color";

const Uint8 bayer4x4[16] = {
	0, 8, 2, 10,
	12, 4, 14, 6,
	3, 11, 1, 9,
	15, 7, 13, 5
...
SDL_Color st_colors[256];
int st_colors_init = 0;
int st_palette_init = 0;
Uint8 st_palette_source[16];
Uint8 st_palette_used[256];
int st_palette_source_count = 16;
Uint8 st_dither_map[16][256];
const Uint8 *st_dither_phase_maps[4][16];
SDL_bool st_shadow_warning_shown = SDL_FALSE;
int st_force_full_refresh = 1;
int st_map_used_count = 0;
...
static void setModeCommon(_THIS, const xbiosmode_t *new_video_mode, int ste_palette);
...
void SDL_XBIOS_VideoInit_ST(_THIS, unsigned long cookie_cvdo)
{
	...
	this->SetColors = SDL_XBIOS_ST_SetColors;

	if ((cookie_cvdo >> 16) == VDO_STE) {
		XBIOS_setMode = setMode_STE;
	}
	if ((cookie_cvdo >> 16) == VDO_ST || (cookie_cvdo >> 16) == VDO_STE) {
		XBIOS_updRects = SDL_XBIOS_ST_UpdateRects;
		this->FlipHWSurface = SDL_XBIOS_ST_FlipHWSurface;
		loadPerfHints();
	}
}
...
static void setMode_ST(_THIS, const xbiosmode_t *new_video_mode)
{
	setModeCommon(this, new_video_mode, 0);
}

static void setMode_STE(_THIS, const xbiosmode_t *new_video_mode)
{
	setModeCommon(this, new_video_mode, 1);
}
